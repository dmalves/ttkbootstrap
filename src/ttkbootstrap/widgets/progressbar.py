...
class Progressbar(Widget, ttk.Progressbar):
    def __init__(self, master=None, bootstyle='primary', orient=HORIZONTAL,
                 **kwargs):
        Widget.__init__(self, bootstyle, 'TProgressbar', orient)
        ttk.Progressbar.__init__(self, master=master, orient=orient, **kwargs)
    ...
    def _configure_bootstyle(self, *args):
        ...
        if not self._theme.style_exists(self._ttkstyle):
            if STRIPED in self._bootstyle:
                builder.create_striped_progressbar_style(self)
            else:
                builder.create_progressbar_style(self)
