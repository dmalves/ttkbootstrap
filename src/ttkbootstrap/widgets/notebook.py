...
# add another style variation that removes the border from the tabs
# while leaving the colors the same as the active tab.


class Notebook(Widget, ttk.Notebook):
    def __init__(self, master=None, bootstyle='default', **kwargs):
        Widget.__init__(self, bootstyle, 'TNotebook')
    ...
    def _configure_bootstyle(self, *args):
        ...
        if not self._theme.style_exists(self._ttkstyle):
            builder.create_notebook_style(self)
